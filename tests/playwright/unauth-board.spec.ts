...
test.describe("Unauthorized access to /api/board", () => {
  test("a logged-out request to /api/board redirects to the homepage", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByRole("button", { name: "Sign In" })).toBeVisible();

    // Same-origin browser fetch, exactly how the app hits the API. Logged out,
    // the protected route should bounce back to the homepage. (Per-method
    // blocking of GET/PUT/DELETE is asserted at the middleware level in
    // tests/vitest/middleware.test.ts.)
